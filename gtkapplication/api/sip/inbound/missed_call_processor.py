...
class InboundProcessor:
	...
	def process_missed_call_message(self,message_body):
		...
		from_phone_number=message_lines[0].split('=')[1]
		self._logger.debug("from_phone_number = %s",from_phone_number)
		db=gtkapplication.api.sip.business_layer.BusinessLayer()
		db.insert_missed_call_message(from_phone_number)
		self._logger.debug("after db insert_missed_call_message()")
		...
		dashboard_ui.add_missed_call_notification(contact,active_contact_point_index)
